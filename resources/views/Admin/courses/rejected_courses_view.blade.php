...
@section('title', 'Pending Course View')

@section('content')
    <br>
    <h2>Rejected Course View</h2>
    ...
    <div class="edit-button">
        <a href="{{ route('publish-course') }}?cid={{ $course->id }}&status={{ $course->publishCourse}}&delete={{ $course->deleteCourse}} " class="btn btn-primary">Publish</a>
        <a href="{{ route('delete-course') }}?cid={{ $course->id }}&status={{ $course->publishCourse}}&delete={{ $course->deleteCourse}}" class="btn btn-danger">Delete</a>
    </div>
